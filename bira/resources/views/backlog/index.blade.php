        ...
        @forelse($boards as $backlogBoard)
            ...
            <div class="flex flex-col gap-4">
                ...
                @if($backlogTasks->isEmpty())
                    <div class="w-full py-8 flex items-center justify-center text-muted-foreground text-sm bg-white/5 border border-dashed border-border-subtle rounded-2xl">
                        No backlog items for this board.
                    </div>
                @else
                    <div class="flex flex-col gap-3 backlog-tasks">
                        @foreach($backlogTasks as $item)
                            @include('boards.tasks._task_card', ['item' => $item, 'backlogBoard' => $backlogBoard, 'permissionLevel' => 'viewer']) {{-- Assuming viewer for global backlog unless otherwise specified --}}
                        @endforeach
                    </div>
                @endif
            </div>
        @empty
            <div class="w-full py-16 flex flex-col items-center justify-center bg-white/5 border border-dashed border-border-subtle rounded-3xl">
